# Jira Timeline Weekend Marker v1.1.0
...
|---|---|---|---|
| `jira-weekend-marker-1.1.0-chrome-edge.zip` | Chrome, Edge, other Chromium browsers | Windows / macOS / Linux | Unzip → `chrome://extensions` or `edge://extensions` → Developer mode → *Load unpacked* |
| `jira-weekend-marker-1.1.0-safari-source.zip` | Safari | macOS | Unzip → `./safari/build-safari.sh` → Run in Xcode → enable in Safari ▸ Settings ▸ Extensions |
| `jira-weekend-marker-1.1.0-safari-macos-app.zip` | Safari (unsigned app) | macOS | Unzip → open the app once → Safari ▸ Develop ▸ *Allow Unsigned Extensions* → enable in Settings ▸ Extensions |

## Notes

- Works in the timeline's **Weeks** zoom level only.
- Upgrading from 1.0.0 keeps your settings (`useKrHolidays` is migrated to `useHolidays`).
- Safari: `storage.sync` behaves like local storage; grant access to `atlassian.net` when prompted.

---

# Jira Timeline Weekend Marker v1.1.0 (한국어)
...
| 파일 | 브라우저 | OS | 설치 |
|---|---|---|---|
| `jira-weekend-marker-1.1.0-chrome-edge.zip` | Chrome, Edge 등 Chromium 계열 | Windows / macOS / Linux | 압축 해제 → `chrome://extensions` 또는 `edge://extensions` → 개발자 모드 → *압축해제된 확장 프로그램을 로드* |
| `jira-weekend-marker-1.1.0-safari-source.zip` | Safari | macOS | 압축 해제 → `./safari/build-safari.sh` → Xcode에서 Run → Safari ▸ 설정 ▸ 확장 프로그램에서 활성화 |
| `jira-weekend-marker-1.1.0-safari-macos-app.zip` | Safari (미서명 앱) | macOS | 압축 해제 → 앱 한 번 실행 → Safari ▸ 개발자용 ▸ *서명되지 않은 확장 프로그램 허용* → 설정 ▸ 확장 프로그램에서 활성화 |

## 참고

- 타임라인의 **주(Weeks)** 단위 보기에서만 동작합니다.
- 1.0.0에서 업그레이드해도 설정이 유지됩니다(`useKrHolidays` → `useHolidays` 자동 이전).
